...
Idea
- Make edges
  - A list of lists [index_i, index_j, manhattan distance between i and j]
- Initialize ans to 0
- Make a minimum spanning tree
  - Sort the edges by distance in an ascending order
  - Iterate edges
    - If two vertices are not connected, union() them.
    - Increment ans by the distance

Complexity
- Let V denote the number of vertices, and E the number of edges
- Time is O(E*logE) for sorting the edges
- Space is O(V) for Union find object to store parent and rank arrays.
"""
